...
| 4 | `validate_ops` | [split_multi_ops.py](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/split_multi_ops.py) | Checks that each op's inputs share the same `ElementArrangement`; runs after layout propagation so the `SpyreTensorLayout`s are available. |
...
| 10 | `chunk_large_tensors` | [chunk_large_tensors.py](https://github.com/torch-spyre/torch-spyre/blob/main/torch_spyre/_inductor/chunk_large_tensors.py) | Gated by `config.chunk_large_tensors`. Splits tensors that exceed the per-core span. |
