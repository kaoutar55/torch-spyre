...
|-------|------|-------------|--------|
| Application / PyTorch | Spyre Extension for PyTorch Profiler | Kernel-level | Planned |
...
| Runtime | Profiling Tools Interface (libaiupti) | Kernel + memory | Planned |
| Device Driver / HW | AIU SMI | Device-level | Planned |
| Post-processing | Holistic Trace Analyser for Spyre | Derived metrics | Planned |
...
A command-line monitoring tool for Spyre devices (similar to
`nvidia-smi`) that will report:
